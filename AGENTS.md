...
- `docs/` holds deeper design, workflow, testing, and runner guidance.
- `.github/agents/` defines specialized review and retrospective behaviors.
...
| New test or change to test infrastructure, test mode, `RunTests()`, or any `*_tests.cpp` file | `docs/testing.md` |
| Change to `build.ps1`, compiler flags, or Windows build steps | `docs/build-and-runner.md` |
| Change to any file in `.github/workflows/` | `docs/workflow.md`, `docs/build-and-runner.md` |
| Change to a gameplay rule, input behavior, win condition, or restart flow | `docs/requirements.md` |
| Change to task, issue, pull request, or review workflow | `docs/workflow.md` |
| Change to review routing logic, review output contract, or `.github/scripts/validate-pr-review.py` | `docs/workflow.md`, `docs/build-and-runner.md` |
| Change to agent prompt scope or behavior in `.github/agents/**` | `docs/workflow.md` |
| Major milestone: new subsystem working end-to-end | `README.md` Current State |
| Change to a core constraint such as platform, memory, testing, build, or dependency policy | `.github/copilot-instructions.md` |

If a change touches multiple rows, update all the listed docs. If a doc update would be a lie, skip it and explain why in the pull request.
