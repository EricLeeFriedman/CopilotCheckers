...
- Build a local two-player checkers game.
...
- Avoid the C standard library as much as practical. Win32 headers are allowed because they are required by the platform.
...
- Each subsystem's tests should live in a dedicated `*_tests.cpp` file once code exists.
...
- Update documentation in the same change when behavior or structure moves.
- Use the doc ownership map in `AGENTS.md` to determine which docs to update.
